...
const agentesEjemplo: Agente[] = [
  {
    id: '1',
    nombre: 'Agente de Correo',
    descripcion: 'Gestiona y responde correos electrónicos automáticamente',
    imagen: '📧',
    habilitado: true,
  },
  {
    ...
    descripcion: 'Asistente inteligente para atención al cliente y ventas',
    imagen: '💼',
    habilitado: false,
  },
  {
    ...
    descripcion: 'Resuelve problemas técnicos y consultas frecuentes',
    imagen: '🛠️',
    habilitado: true,
  },
  {
    ...
    descripcion: 'Ayuda en estrategias de marketing y contenido',
    imagen: '📊',
    habilitado: false,
  },
  {
    ...
    descripcion: 'Asistente para gestión de RRHH y procesos internos',
    imagen: '👥',
    habilitado: true,
  },
  {
    ...
    descripcion: 'Analiza datos y genera reportes automatizados',
    imagen: '📈',
    habilitado: false,
  },
  {
    id: '7',
    nombre: 'Agente de Traducción',
    descripcion: 'Traduce contenido entre múltiples idiomas',
    imagen: '🌐',
    habilitado: true,
  },
  {
    id: '8',
    nombre: 'Agente de Programación',
    descripcion: 'Asistente para desarrollo y debugging de código',
    imagen: '💻',
    habilitado: false,
  },
];
...
export default function MisAgentesPage() {
  ...
  const [agentes, setAgentes] = useState<Agente[]>(agentesEjemplo);
  ...
  const toggleAgente = (id: string) => {
    setAgentes((prev) =>
      prev.map((agente) =>
        agente.id === id ? { ...agente, habilitado: !agente.habilitado } : agente
      )
    );
  };
    ...
    <div className="flex min-h-screen bg-gray-50">
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <nav className="flex-1 space-y-1 px-3 py-4">
            ...
            <a href="/mis-agentes" className="group flex items-center rounded-lg px-3 py-2.5 text-sm font-medium text-blue-600 bg-blue-50 transition-colors">
      ...
      <div className="flex-1 flex flex-col lg:ml-0">
        ...
        <main className="flex-1 px-4 py-8 sm:px-6 lg:px-8">
          <div className="mb-6">
            <h2 className="text-3xl font-bold text-gray-900 mb-2">Mis Agentes</h2>
            <p className="text-gray-600">Gestiona y habilita tus agentes inteligentes</p>
          </div>
          ...
          <div className="mb-6">
            <div className="relative max-w-md">
              ...
              <input
                ...
                className="w-full rounded-lg border border-gray-300 bg-white pl-10 pr-4 py-2.5 text-gray-900 placeholder-gray-400 focus:border-gray-400 focus:outline-none focus:ring-1 focus:ring-gray-400"
              />
            </div>
          </div>
          ...
          {filteredAgentes.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-gray-500 text-lg">No se encontraron agentes que coincidan con "{searchQuery}"</p>
            </div>
          ) : (
            <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
              {filteredAgentes.map((agente) => (
                <div key={agente.id} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-start justify-between mb-4">
                    <div className="text-5xl mb-3">{agente.imagen}</div>
                    <button
                      onClick={() => toggleAgente(agente.id)}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
                        agente.habilitado ? 'bg-blue-600' : 'bg-gray-300'
                      }`}
                    >
                      <span
                        className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                          agente.habilitado ? 'translate-x-6' : 'translate-x-1'
                        }`}
                      />
                    </button>
                  </div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">{agente.nombre}</h3>
                  <p className="text-gray-600 text-sm mb-4">{agente.descripcion}</p>
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-medium px-2.5 py-1 rounded-full ${
                      ...
                      {agente.habilitado ? 'Habilitado' : 'Deshabilitado'}
                    </span>
                  </div>
                </div>
              ))}
